...
BEGIN
  ...
  SELECT jsonb_build_object(
    'mission_data', jsonb_build_object(
      ...
      -- Donneur d'ordre (créateur de la mission)
      'donneur_ordre_name', (
        SELECT COALESCE(p.full_name, p.first_name || ' ' || p.last_name, p.email)
        FROM profiles p WHERE p.id = m.user_id
      ),
      'donneur_ordre_company', (
        SELECT COALESCE(NULLIF(p.company_name, ''), NULLIF(p.company, ''))
        FROM profiles p WHERE p.id = m.user_id
      ),
      -- Convoyeur (sans téléphone)

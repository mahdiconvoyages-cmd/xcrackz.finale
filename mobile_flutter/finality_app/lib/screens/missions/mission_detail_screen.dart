...
class _MissionDetailScreenState extends ConsumerState<MissionDetailScreen> {
  ...
  BackgroundTrackingService get _trackingService => ref.read(backgroundTrackingServiceProvider);
  Mission? _mission;
    ...
    try {
      ...
      try {
        liftOffer = await Supabase.instance.client
            ...
            .maybeSingle();
        if (liftOffer != null) {
          final matchResult = await Supabase.instance.client
              .from('ride_matches')
              .select('id')
      ...
      setState(() {
        ...
        // Share code is auto-generated by SQL trigger — no link generation needed
      });
    ...
    try {
      if (_isTrackingActive) {
        ...
        if (started && mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Impossible de demarrer le GPS.\n'
                  'Verifiez que la localisation est activee dans les parametres.'),
              backgroundColor: PremiumTheme.accentRed,
              duration: Duration(seconds: 4),
            ),
          );
  ...
  // ── Partage tracking au client (système par code) ──

  Widget _buildClientTrackingShareCard() {
    final shareCode = _mission?.shareCode;
    final hasCode = shareCode != null && shareCode.isNotEmpty;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _isTrackingActive ? PremiumTheme.accentGreen.withValues(alpha: 0.5) : PremiumTheme.primaryBlue.withValues(alpha: 0.4),
        ),
        boxShadow: [
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _isTrackingActive ? 'Position mise à jour en continu' : 'Partagez le code au client',
          ...
          if (!hasCode) ...[  
            // No share code (should not happen, but fallback)
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
              ...
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Expanded(child: Text('Code de suivi non disponible', style: TextStyle(fontSize: 13, color: Colors.orange))),
                ],
              ),
            ),
          ] else ...[
            // Show tracking URL + code
            Container(
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      const Icon(Icons.language, size: 16, color: PremiumTheme.primaryBlue),
                      const SizedBox(width: 8),
                      const Expanded(
                        child: Text(
                          'checksfleet.com/suivi',
                          style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: PremiumTheme.primaryBlue),
                        ),
                      ),
                      IconButton(
                        onPressed: () => _copyToClipboard('https://checksfleet.com/suivi'),
                  ...
                  Row(
                    children: [
                      const Icon(Icons.pin, size: 16, color: PremiumTheme.accentGreen),
                      const SizedBox(width: 8),
                      Text(
                        'Code : $shareCode',
                        style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: PremiumTheme.textPrimary, letterSpacing: 1.5),
                      ),
                      const Spacer(),
                      IconButton(
                        onPressed: () => _copyToClipboard(shareCode),
                        icon: const Icon(Icons.copy, size: 16, color: PremiumTheme.accentGreen),
  ...
  String _buildTrackingMessage() {
    ...
    final code = _mission?.shareCode ?? '';
    return 'Bonjour,\n\n'
        'Suivez la livraison de votre vehicule en temps reel :\n\n'
        '${ref.isNotEmpty ? "Mission : $ref\n" : ""}'
        '1. Rendez-vous sur : https://checksfleet.com/suivi\n'
        '${code.isNotEmpty ? "2. Entrez le code : $code\n" : ""}'
        '\nChecksFleet';
  }

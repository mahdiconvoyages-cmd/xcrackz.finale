...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  payment_method text, -- 'manual', 'stripe', 'mollie', etc.
  assigned_by uuid REFERENCES profiles(id), -- Admin qui a assigné l'abonnement manuellement
  notes text,
...
CREATE TABLE IF NOT EXISTS subscription_history (
  ...
  old_status text,
  new_status text,
  changed_by uuid REFERENCES profiles(id), -- Admin ou système
  reason text,
  created_at timestamptz DEFAULT now()
);
...
-- Les admins peuvent créer/modifier des abonnements
...
-- Les admins peuvent créer des entrées d'historique
...
-- Créer des abonnements free par défaut pour les utilisateurs existants
INSERT INTO subscriptions (user_id, plan, status, current_period_end, payment_method)
SELECT
  id,
  'free',
  'active',
...
-- Ajouter une colonne banned aux profiles si elle n'existe pas
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Ajouter une colonne ban_reason aux profiles si elle n'existe pas
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Ajouter une colonne banned_at aux profiles si elle n'existe pas
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns

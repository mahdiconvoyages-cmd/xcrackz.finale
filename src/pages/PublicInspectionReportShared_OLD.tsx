/**
 * 📄 Page Publique de Rapport d'Inspection Partagé - VERSION PREMIUM
 * 
 * Accessible sans authentification via lien unique
 * Design ultra-professionnel pour clients
 */
...
import {
  ...
  Download, ZoomIn, Shield, Wrench, Package, FileSignature, Camera, 
  ChevronUp, ChevronDown, User, Phone, Clock, Gauge, Fuel, Image as ImageIcon,
  Navigation, Archive, Printer
} from 'lucide-react';
...
export default function PublicInspectionReportShared() {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const photos: any[] = [];
      if (data.inspection_departure?.photos) {
        photos.push(...data.inspection_departure.photos.map((p: any) => ({ ...p, type: 'Départ' })));
      }
      if (data.inspection_arrival?.photos) {
        photos.push(...data.inspection_arrival.photos.map((p: any) => ({ ...p, type: 'Arrivée' })));
      }
      setAllPhotos(photos);
  ...
  const nextPhoto = () => {
    const photos = selectedPhoto?.includes('departure') || allPhotos[photoIndex]?.type === 'Départ' 
      ? reportData.inspection_departure?.photos 
      : reportData.inspection_arrival?.photos;
    if (photos && photoIndex < photos.length - 1) {
      setPhotoIndex(photoIndex + 1);
      setSelectedPhoto(photos[photoIndex + 1]?.url || photos[photoIndex + 1]?.photo_url);
    }
  };
  ...
  const handleDownloadAllPhotos = async () => {
    try {
      ...
      // Télécharger photos de départ
      if (reportData.inspection_departure?.photos) {
        const departFolder = zip.folder('Photos_Depart');
        for (const photo of reportData.inspection_departure.photos) {
          const url = photo.url || photo.photo_url;
          if (url) {
            const response = await fetch(url);
            const blob = await response.blob();
            departFolder?.file(`depart_${++photoCount}.jpg`, blob);
          }
        }
      }
      
      // Télécharger photos d'arrivée
      if (reportData.inspection_arrival?.photos) {
        const arrivalFolder = zip.folder('Photos_Arrivee');
        photoCount = 0;
        for (const photo of reportData.inspection_arrival.photos) {
          const url = photo.url || photo.photo_url;
          if (url) {
            const response = await fetch(url);
            const blob = await response.blob();
            arrivalFolder?.file(`arrivee_${++photoCount}.jpg`, blob);
          }
        }
      }
      
      const content = await zip.generateAsync({ type: 'blob' });
      saveAs(content, `Inspection_${reportData.mission_data?.reference || 'rapport'}_Photos.zip`);

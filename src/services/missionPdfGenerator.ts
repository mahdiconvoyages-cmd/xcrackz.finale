...
// ✅ jsPDF 3.x built-in Helvetica supports WinAnsiEncoding (accented chars, €, °, etc.)
// Only strip emojis and characters outside WinAnsiEncoding range
...
export async function generateMissionPDF(mission: Mission, assignment?: Assignment) {
  ...
  doc.text(`Créé le: ${new Date(mission.created_at).toLocaleString('fr-FR')}`, pageWidth - margin, 25, { align: 'right' });
  // doc.setGState(doc.GState({ opacity: 1 }));
  ...
  doc.text('VÉHICULE', margin + 5, y + 8);
  ...
  if (mission.price > 0) {
    ...
    doc.text(`${mission.price.toFixed(2)} €`, pageWidth - margin - 5, y - 8, { align: 'right' });
  }
  ...
  if (assignment) {
    ...
    if (assignment.assigner?.email) {
      doc.text(`Assigné par: ${cleanText(assignment.assigner.email)}`, margin + 5, y + 7);
    }
    ...
    if (assignment.payment_ht) {
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(colors.success[0], colors.success[1], colors.success[2]);
      doc.text(`Paiement HT: ${assignment.payment_ht.toFixed(2)} €`, margin + 5, y + 21);
    }

    if (assignment.commission) {
      doc.setTextColor(colors.accent[0], colors.accent[1], colors.accent[2]);
      doc.text(`Commission: ${assignment.commission.toFixed(2)} €`, pageWidth - margin - 5, y + 21, { align: 'right' });
    }
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(colors.gray[0], colors.gray[1], colors.gray[2]);
  doc.text('CHECKSFLEET - Solution de gestion de transport', pageWidth / 2, footerY + 5, { align: 'center' });
  doc.text(`Page 1/1`, pageWidth - margin, footerY + 5, { align: 'right' });
  doc.text(`Généré le ${new Date().toLocaleDateString('fr-FR')}`, margin, footerY + 5);

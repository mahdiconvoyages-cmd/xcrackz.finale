...
function generateCoverPage(pdf: jsPDF, report: InspectionReportComplete, type: string) {
  ...
  pdf.setFontSize(22);
  pdf.setFont('helvetica', 'bold');
  const title = type === 'complete' ? 'RAPPORT COMPLET' 
    : type === 'departure' ? 'INSPECTION DÉPART' 
    : type === 'arrival' ? 'INSPECTION ARRIVÉE'
    : type === 'restitution_departure' ? 'INSPECTION DÉPART RESTITUTION'
    : type === 'restitution_arrival' ? 'INSPECTION ARRIVÉE RESTITUTION'
    : type === 'restitution_complete' ? 'RAPPORT COMPLET (RESTITUTION INCLUSE)'
...
async function generateInspectionPages(pdf: jsPDF, inspection: InspectionDetails, type: string, report: InspectionReportComplete, pageNum: number, totalPages: number) {
  ...
  y = addSection(pdf, y, 'Équipements & Accessoires', '🔧', C.primary);
  
  // Keys count
  const keysCount = vi.keys_count ?? inspection.keys_count;
  if (keysCount !== undefined) {
    y = addTable(pdf, y, [['Nombre de clés', String(keysCount)]]);
    y += 2;
  }
  
  y = addCheckTable(pdf, y, [
    ['Kit de sécurité', vi.has_security_kit ?? inspection.has_security_kit ?? false],
    ['Roue de secours', vi.has_spare_wheel ?? inspection.has_spare_wheel ?? false],
    ['Kit de gonflage', vi.has_inflation_kit ?? inspection.has_inflation_kit ?? false],
    ['Carte carburant', vi.has_fuel_card ?? inspection.has_fuel_card ?? false],
    ['Véhicule chargé', vi.is_loaded ?? inspection.is_loaded ?? false],
  ...
  if (vi.keys_returned !== undefined) {
    y += 3;
    y = addCheckTable(pdf, y, [
      ['Clés restituées', vi.keys_returned ?? false],
      ['Documents restitués', vi.documents_returned ?? false],
    ]);
  }
  y += 5;

  // === DOCUMENTS SCANNÉS ===
  if (inspection.scanned_documents && inspection.scanned_documents.length > 0) {
    y = addSection(pdf, y, 'Documents Scannés', '📄', C.primary);
    const docsData: Array<[string, string]> = inspection.scanned_documents.map((doc: any, i: number) => [
      doc.title || doc.document_title || `Document ${i + 1}`,
      doc.file_url || doc.document_url ? '✓ Téléchargeable' : '—'
    ]);
    y = addTable(pdf, y, docsData);
    y += 5;
  }
...
export async function generatePremiumInspectionPDF(
  ...
  try {
    ...
    if ((type === 'arrival' || type === 'complete' || type === 'restitution_complete') && report.arrival) {
      pageNum = await generateInspectionPages(pdf, report.arrival, 'arrival', report, pageNum, totalPages);
    }
    
    // Pages d'inspection restitution
    if ((type === 'restitution_departure' || type === 'restitution_complete' || type === 'complete') && report.restitutionDeparture) {
      pageNum = await generateInspectionPages(pdf, report.restitutionDeparture, 'departure', report, pageNum, totalPages);
    }
    
    if ((type === 'restitution_arrival' || type === 'restitution_complete' || type === 'complete') && report.restitutionArrival) {
      pageNum = await generateInspectionPages(pdf, report.restitutionArrival, 'arrival', report, pageNum, totalPages);
    }
